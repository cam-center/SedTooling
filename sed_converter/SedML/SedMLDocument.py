...
class SedMLDocument:
    ...
    def _processDocument(self):
        ...
        self._processDataGens()
        self._processVariablesAndParams()
        self._processTasks()
        ...
        for output in self.output_list:
            if isinstance(output, SedMLPlot):
                if isinstance(output, SedMLPlot2D):
                    for i in range(0, output.getNumCurves()):
                ...
                if isinstance(output, SedMLPlot3D):
                    for i in range(0, output.getNumSurfaces()):
    ...
    def _processVariablesAndParams(self):
    ...
    def __delve_into_repeated_task(self,
                                   repeated_task: SedMLRepeatedTask) -> tuple[set[SedMLModel], set[SedMLSimulation]]:
